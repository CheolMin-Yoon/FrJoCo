...
class HierarchicalWholeBodyController():
    ...
    def _task_waist_constraint(self, waist_dof_id, N_prev, dq_prev):
        ...
        lambda_val = 1e-4
        J_proj_pinv = J_projected.T / (J_projected @ J_projected.T + lambda_val)
    ...
    def _task_static_contact(self, stance_foot_id, N_prev, dq_prev):
        ...
        J_proj_pinv = J_projected.T @ np.linalg.inv(
            J_projected @ J_projected.T + lambda_val * np.eye(3)
        )
    ...
    def _task_floating_base(self, torso_body_id, ref_torso_pos, N_prev, dq_prev):
        ...
        lambda_val = 1e-4
        J_proj_pinv = J_projected.T @ np.linalg.inv(
            J_projected @ J_projected.T + lambda_val * np.eye(6)
        )
    ...
    def _task_swing_leg(self, swing_foot_id, ref_pos, ref_quat=None, 
        ...
        if N_prev is None:
            N_prev = np.eye(self.nv)
        if dq_prev is None:
            dq_prev = np.zeros(self.nv)
        # 1. Jacobian 계산
        J = np.zeros((6, self.nv))
    ...
    def DynamicsWBC(self, q_cmd, dq_cmd, fr_left, fr_right, left_foot_id, right_foot_id, dt):
        ...
        # Step 3: Contact Jacobian (contact 발만)
        ...
        # 어느 발이 contact인지 판별 (fz > 0 이면 contact)
        ...
        # contact 발에만 마찰 원뿔 적용, swing 발은 f=0 등식 제약
        friction_rows = []
        h_rows = []
        
        for is_contact, offset in [(left_contact, 0), (right_contact, 3)]:
            if is_contact:
                block = np.zeros((5, 6))
                block[:, offset:offset+3] = C_friction_single
                friction_rows.append(block)
                h_rows.extend([0.0] * 5)
            else:
                # swing 발: fx=fy=fz=0 (등식을 부등식 쌍으로 표현)
                for i in range(3):
                    row_pos = np.zeros((1, 6)); row_pos[0, offset+i] =  1.0
                    row_neg = np.zeros((1, 6)); row_neg[0, offset+i] = -1.0
                    friction_rows.extend([row_pos, row_neg])
                    h_rows.extend([0.0, 0.0])
        
        C_friction = np.vstack(friction_rows)  # (n_ineq, 6)
        n_ineq = C_friction.shape[0]
        
        G = np.hstack([np.zeros((n_ineq, self.nv)), C_friction])
        h = np.array(h_rows)
